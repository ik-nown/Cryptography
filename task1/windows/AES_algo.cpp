...
#include "assert.h"
#include <chrono>
#include <D:/School/HK3/MMH/Labs/lab2/AES/task/task1/AES_algo.h>
// Cryptopp Librari
...
void AES_algo::encryptAES()
{
    ...
    else if (this->mode == "CCM")
    {
        ...
        e.SetKeyWithIV(key, sizeof(key), newIV, sizeof(newIV));
        e.SpecifyDataLengths(0, str_pl.size(), 0);
        StringSource ss(str_pl, true, new AuthenticatedEncryptionFilter(e, new StringSink(str_ct)));
        cipher = str_ct;
        for (int i=0; i< 1000; i++) {
            StringSource ss(str_pl, true, new AuthenticatedEncryptionFilter(e, new StringSink(str_ct)));
        }
    ...
    else if (this->mode == "GCM")
    {
        ...
        e.SetKeyWithIV(this->key, AES::DEFAULT_KEYLENGTH, iv, AES::BLOCKSIZE);
...
void AES_algo::decryptAES(wstring hexCipher)
{
    ...
    if (this->mode == "GCM")
    {
        ...
        StringSource(str_ct, true, new AuthenticatedDecryptionFilter(d, new StringSink(str_pl)));
        plain = str_pl;
        for (int i=0; i<1000; i++) {
            StringSource(str_ct, true, new AuthenticatedDecryptionFilter(d, new StringSink(str_pl)));
        }
